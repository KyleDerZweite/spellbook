...
class ImportExportService:
    ...
    async def import_csv(
        ...
        logger.info(f"Import completed: {results['successful']}/{results['total_lines']} successful")
        return results
    ...
    async def export_csv(
        ...
        for cc in collection_cards:
            ...
            row = {
                ...
                'Foil': 'Yes' if cc.condition and 'foil' in cc.condition.lower() else 'No',
                'Language': 'en'
            }
    ...
    async def export_json(
        ...
        for cc in collection_cards:
            ...
            card_data = {
                'scryfall_id': str(cc.card.scryfall_id) if cc.card.scryfall_id else None,
    ...
    def _normalize_row(self, row: Dict[str, str], column_map: Dict[str, str]) -> Dict[str, Any]:
        ...
        for csv_col, internal_col in column_map.items():
            if csv_col in row:
                ...
                if internal_col == 'quantity':
                    ...
                    normalized[internal_col] = self.CONDITION_MAP.get(value.lower(), value)
                elif internal_col == 'foil':
                    normalized[internal_col] = value.lower() in ('yes', 'true', '1', 'foil')
                else:
