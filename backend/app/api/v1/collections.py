...
async def get_or_create_default_collection(db: AsyncSession, user: User) -> Collection:
    ...
    result = await db.execute(
        select(Collection).where(
            Collection.user_id == user.id, Collection.name == "My Collection"
        )
    )
    ...
    if not collection:
        collection = Collection(
            name="My Collection", description="Default card collection", user_id=user.id
        )
        db.add(collection)
        await db.commit()
...
async def add_card_to_my_collection(
    ...
    await card_service.make_card_permanent(
        db_card.scryfall_id, CardStorageReason.USER_COLLECTION, db, current_user.id
    )
...
async def update_card_quantity_in_my_collection(
    ...
    if not db_card:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Card not found in collection"
        )
...
@router.post(
    "/{collection_id}/cards",
    response_model=CollectionCardResponse,
...
@router.delete(
    "/{collection_id}/cards/{card_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
async def import_to_collection(
    ...
    format: Optional[str] = Form(
        None, description="Format hint: csv, deckbox, moxfield, arena, json"
    ),
    db: AsyncSession = Depends(get_async_session),
    current_user: User = Depends(get_current_user),
):
...
async def export_collection(
    ...
    include_details: bool = Query(
        False, description="Include full card details (JSON only)"
    ),
    db: AsyncSession = Depends(get_async_session),
    current_user: User = Depends(get_current_user),
):
