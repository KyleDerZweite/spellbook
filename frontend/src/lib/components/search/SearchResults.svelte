	...
	{#if error}
		...
		<div class="flex items-center justify-center py-20">
			<div class="text-center">
				...
				<p class="mt-1 font-body text-xs italic text-text-muted">Try again or adjust your search.</p>
			</div>
		</div>
	{:else if loading && hits.length === 0}
		...
		>
			{#each Array(SKELETON_COUNT) as _, i}
				<div class="overflow-hidden rounded" style="background-color: var(--color-stone); border: 1px solid rgba(196, 146, 42, 0.12);">
					<div
						style="aspect-ratio: 5 / 7; background-color: var(--color-slate); animation: skeleton-pulse 1.5s ease-in-out infinite; animation-delay: {i * 50}ms;"
					></div>
					<div class="px-2 py-2">
						<div
							class="mb-1 h-3 rounded"
							style="width: {60 + (i % 3) * 15}%; background-color: var(--color-slate); animation: skeleton-pulse 1.5s ease-in-out infinite; animation-delay: {i * 50 + 100}ms;"
						></div>
						<div
							class="h-2.5 rounded"
							style="width: 40%; background-color: var(--color-slate); animation: skeleton-pulse 1.5s ease-in-out infinite; animation-delay: {i * 50 + 200}ms;"
						></div>
					</div>
				</div>
			{/each}
